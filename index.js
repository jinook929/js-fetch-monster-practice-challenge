...
const getMonsters = a => { 
  ...
  fetch(URL_PREFIX + `monsters/?_sort=id&_order=desc&_limit=50&_page=${a}`)
  .then(b => b.json())
  .then(b => { 
    console.log("getMonsters (latest id first) =>", b)
    document.querySelector('#monster-container').innerHTML = ''; 
    for (let c = 0; c < b.length; c++) 
      // console.log('monster', b[c]), 
      createMonsterCard(b[c]) 
      // createMonsterCard(sorted[c]) 
  }) 
}, 
...
createMonsterForm = () => { 
  ...
  e.innerHTML = 'Create', a.appendChild(b), 
  a.appendChild(c), 
  a.appendChild(d), 
  a.appendChild(e), 
...
addSubmitEventListener = () => { 
  document.querySelector('#monster-form').addEventListener('submit', a => { 
    ...
    if(getFormData().name === "" || Number.isNaN(getFormData().age) || getFormData().description === "") {alert("Fill in the boxes.")} 
    else {postNewMonster(getFormData()),clearForm()}
  }) 
}, 
